...
class RabbitMqSinkBuilder extends SinkFactory.SinkBuilder {
    ...
    public EventSink build(Context context, String... args) {
        if (!(args.length >= 1) )
		throw new IllegalArgumentException("Missing arguments: queue host,[queue-domain=\"\"],[username=\"guest\"],[password=\"guest\"],[exchange=\"\"]");
	...
	String exchange = args.length>=5? args[4]: "";
		return new RabbitMqSink(new SimpleRabbitMqProducer(publisher, userName, password, exchange),queueDomain);
    }
}
